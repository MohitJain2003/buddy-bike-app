...
const EditBike = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let urls = [...existingImages]; // keep existing by default unless user uploads new

      if (files.length > 0) {
        urls = []; // if they upload new, replace old
  ...
  return (
    <>
      ...
      <section className="admin-section" style={{ maxWidth: '700px', margin: '40px auto', padding: '20px' }}>
        <h2 className="glow page-title" style={{ marginBottom: '30px', textAlign: 'center' }}>Edit Bike</h2>
        ...
        <div className="bike-card" style={{ padding: '30px', background: '#0a0a0a', textAlign: 'left' }}>
          <form onSubmit={handleSubmit}>
            ...
            <div className="input-group" style={{ marginTop: '15px' }}>
              <label style={{ display: 'block', marginBottom: '8px', color: '#ddd', fontWeight: 'bold' }}>Update Images (Optional - Overwrites old)</label>
              <input type="file" className="custom-input" multiple style={{ padding: '10px' }} onChange={(e) => setFiles(Array.from(e.target.files))} />
              {existingImages.length > 0 && files.length === 0 && (
                <p style={{ color: '#aaa', fontSize: '12px', marginTop: '5px' }}>Currently has {existingImages.length} images uploaded.</p>
              )}
            </div>
